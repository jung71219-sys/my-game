...
class GameTracker(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"遊戲練功效率紀錄工具 v{self.CURRENT_VERSION}")
    ...
    def setup_record_tab(self):
        ...
        test_update_btn = QPushButton("🔍 測試更新")
        test_update_btn.setFixedWidth(100)
        test_update_btn.clicked.connect(self.test_update_connection)
        ...
        tool_layout.addWidget(test_update_btn)
        ...
        self.time_input.setRange(0.00, 999999); self.time_input.setDecimals(2); self.time_input.setSuffix(" 分鐘")
        ...
        self.alarm_btn = QPushButton("設定鬧鐘")
        self.alarm_btn.setStyleSheet("background-color: #FF9800; color: white; font-weight: bold; height: 35px;")
        ...
        btn_layout.addWidget(self.alarm_btn)
        ...
        self.setup_enter_to_add_record()
    
    def setup_enter_to_add_record(self):
        line_edits = [
            self.crit_input,
            self.atk_boost_input,
            self.note_input,
            self.dragon_dmg_input
        ]
        
        spin_boxes = [
            self.time_input,
            self.exp_start_input,
            self.exp_end_input,
            self.c_cross_score_input
        ]
        ...
        for line_edit in line_edits:
            line_edit.returnPressed.connect(self.add_record_by_enter)
        
        for spin_box in spin_boxes:
            spin_box.lineEdit().returnPressed.connect(self.add_record_by_enter)
        
        for combo in editable_combos:
            if combo.isEditable() and combo.lineEdit():
                combo.lineEdit().returnPressed.connect(self.add_record_by_enter)
    
    def add_record_by_enter(self):
        if self.tabs.currentWidget() != self.record_tab:
            return
        ...
        if focus_widget in excluded_widgets:
            return
    ...
    def toggle_alarm(self):
        if self.alarm_enabled:
            self.cancel_alarm()
            self.statusBar().showMessage("鬧鐘已取消", 3000)
            return
        
        target_minutes = self.time_input.value()
        if target_minutes <= 0:
            QMessageBox.warning(self, "鬧鐘設定", "請先在「時長」欄位輸入要提醒的分鐘數。")
            return
        
        self.alarm_enabled = True
        self.alarm_triggered = False
        self.alarm_target_minutes = target_minutes
        self.alarm_start_time = time.time()
        self.alarm_timer.start(1000)
        self.update_alarm_button_style()
        self.update_alarm_button_text(target_minutes)
        self.statusBar().showMessage(f"鬧鐘已設定：{target_minutes:.2f} 分鐘後提醒", 5000)
    ...
    def check_alarm(self):
        ...
        if self.alarm_target_minutes <= 0:
            return
        
        if self.alarm_start_time is None:
            return
        
        elapsed_mins = (time.time() - self.alarm_start_time) / 60
        remaining_mins = self.alarm_target_minutes - elapsed_mins
        
        if remaining_mins > 0:
            self.update_alarm_button_text(remaining_mins)
            return
        ...
        original_target = self.alarm_target_minutes
        self.alarm_target_minutes = 0
        self.alarm_start_time = None
        
        self.alarm_btn.setText("設定鬧鐘")
        self.update_alarm_button_style()
        self.statusBar().showMessage("測試時間到了！", 5000)
        ...
        QMessageBox.information(
            self,
            "鬧鐘提醒",
            f"測試時間到了！\n設定時間：{original_target:.2f} 分鐘\n實際經過：{elapsed_mins:.2f} 分鐘"
        )
    
    def update_alarm_button_text(self, minutes):
        if minutes >= 1:
            self.alarm_btn.setText(f"取消鬧鐘({minutes:.0f}m)")
        else:
            seconds = max(0, int(minutes * 60))
            self.alarm_btn.setText(f"取消鬧鐘({seconds}s)")
    
    def update_alarm_button_style(self):
        if not hasattr(self, "alarm_btn"):
            return
        
        if self.alarm_enabled:
            self.alarm_btn.setStyleSheet("background-color: #E91E63; color: white; font-weight: bold; height: 35px;")
        else:
            self.alarm_btn.setStyleSheet("background-color: #FF9800; color: white; font-weight: bold; height: 35px;")
    ...
    def auto_check_update(self):
        ...
        try:
            print(f"[更新檢查] 開始檢查更新...")
            print(f"[更新檢查] URL: {self.UPDATE_URL}")
            
            response = requests.get(self.UPDATE_URL, timeout=10)
            print(f"[更新檢查] HTTP 狀態碼: {response.status_code}")
            
            if response.status_code == 200:
                data = response.json()
                print(f"[更新檢查] 成功取得資料: {data.keys()}")
                ...
                if "equip_data" in data:
                    ...
                    for key in self.equip_data:
                        if key in remote_equip:
                            before_count = len(self.equip_data[key])
                            combined = list(set(self.equip_data[key] + remote_equip[key]))
                            self.equip_data[key] = combined
                            after_count = len(self.equip_data[key])
                            if after_count > before_count:
                                updated_count += (after_count - before_count)
                                print(f"[更新檢查] {key} 新增了 {after_count - before_count} 個裝備")
                    
                    if updated_count > 0:
                        self.refresh_all_combos()
                        self.update_config_table_from_data()
                        self.save_data()
                        print(f"[更新檢查] 總共同步了 {updated_count} 個新裝備")
                        QMessageBox.information(
                            self, "裝備資料已更新", 
                            f"已從雲端同步 {updated_count} 個新裝備項目！"
                        )
                
                remote_version = data.get("version", self.CURRENT_VERSION)
                print(f"[更新檢查] 本地版本: {self.CURRENT_VERSION}, 遠端版本: {remote_version}")
                
                remote_v_list = [int(x) for x in remote_version.split(".")]
                current_v_list = [int(x) for x in self.CURRENT_VERSION.split(".")]
                
                if remote_v_list > current_v_list:
                    print(f"[更新檢查] 發現新版本！")
                    reply = QMessageBox.information(
                        self, "發現新版本", 
                        f"目前版本: v{self.CURRENT_VERSION}\n最新版本: v{remote_version}\n\n"
            ...
            error_msg = "連線逾時，請檢查網路連線"
            print(f"[更新檢查] 錯誤: {error_msg}")
            QMessageBox.warning(self, "更新檢查失敗", error_msg)
        
        except requests.exceptions.ConnectionError:
            error_msg = "無法連線到 GitHub 伺服器，請檢查網路或防火牆設定"
            print(f"[更新檢查] 錯誤: {error_msg}")
            QMessageBox.warning(self, "更新檢查失敗", error_msg)
    ...
    def test_update_connection(self):
        print("\n" + "="*60)
        print("開始測試 GitHub 連線")
        print("="*60)
        
        try:
            print(f"[1] 測試 URL: {self.UPDATE_URL}")
            response = requests.get(self.UPDATE_URL, timeout=10)
            print(f"[2] HTTP 狀態碼: {response.status_code}")
            
            if response.status_code == 200:
                print(f"[3] 回應內容長度: {len(response.text)} bytes")
                data = response.json()
                print(f"[4] JSON 鍵值: {list(data.keys())}")
                
                info = f"""✅ 連線測試成功！



遠端版本: {data.get('version', '未知')}
本地版本: {self.CURRENT_VERSION}
...
遠端裝備資料:"""
                ...
                if "equip_data" in data:
                    for key, items in data["equip_data"].items():
                        info += f"\n  {key}: {len(items)} 個"
                
                QMessageBox.information(self, "連線測試成功", info)
                print("="*60)
                print("測試完成 - 成功")
                print("="*60 + "\n")
            else:
            ...
            error_msg = f"連線測試失敗：\n{str(e)}"
            QMessageBox.critical(self, "連線測試失敗", error_msg)
...
""")
    ...
    def copy_last_record(self):
        ...
        try:
            ...
            end_item = self.table.item(last_row, 13)
            if end_item and end_item.text().strip():
                prev_end = float(end_item.text())
                self.exp_start_input.setValue(prev_end)
            
            self.exp_end_input.setValue(0)
            self.time_input.setValue(0)
            self.statusBar().showMessage("已複製前一筆紀錄", 3000)
        
        except Exception as e:
            QMessageBox.warning(self, "複製失敗", f"複製前一筆紀錄時發生錯誤：\n{str(e)}")
    ...
    def add_record(self):
        try:
            ...
            self.statusBar().showMessage("已新增紀錄", 3000)
        except Exception as e:
    ...
    def edit_record(self):
        ...
        if row >= 0:
            ...
            self.apply_conditional_formatting()
            self.update_analysis()
            self.statusBar().showMessage("已修改選中列", 3000)
        else:
    ...
    def load_to_inputs_by_row(self, row):
        ...
        try:
            w_p, w_v = split_plus(get_text(0))
            self.weapon_plus.setCurrentText(w_p)
            self.weapon_input.setCurrentText(w_v)
            
            n_p, n_v = split_plus(get_text(1))
            self.neck_plus.setCurrentText(n_p)
            self.neck_input.setCurrentText(n_v)
            
            cp1, cv1 = split_plus(get_text(2))
            self.card_plus1.setCurrentText(cp1)
            self.card_input1.setCurrentText(cv1)
            
            cp2, cv2 = split_plus(get_text(3))
            self.card_plus2.setCurrentText(cp2)
            self.card_input2.setCurrentText(cv2)
            
            cp3, cv3 = split_plus(get_text(4))
            self.card_plus3.setCurrentText(cp3)
            self.card_input3.setCurrentText(cv3)
            
            cp4, cv4 = split_plus(get_text(5))
            self.card_plus4.setCurrentText(cp4)
            self.card_input4.setCurrentText(cv4)
            
            m_p, m_v = split_plus(get_text(6))
            self.mount_plus.setCurrentText(m_p)
            self.mount_input.setCurrentText(m_v)
            
            s_p, s_v = split_plus(get_text(7))
            self.soul_plus.setCurrentText(s_p)
            self.soul_input.setCurrentText(s_v)
            
            self.pet_input.setCurrentText(get_text(8))
            self.crit_input.setText(get_text(9))
            self.atk_boost_input.setText(get_text(10))
            
            try:
                self.time_input.setValue(float(get_text(11)))
            except:
                self.time_input.setValue(0)
            
            try:
                self.exp_start_input.setValue(float(get_text(12)))
            except:
                self.exp_start_input.setValue(0)
            
            try:
                self.exp_end_input.setValue(float(get_text(13)))
            except:
                self.exp_end_input.setValue(0)
            
            dragon_text = get_text(16)
            self.dragon_time_combo.setCurrentIndex(0)
            self.dragon_dmg_input.clear()
            ...
            if dragon_text.startswith("(") and ")" in dragon_text:
                ...
                self.dragon_dmg_input.setText(dragon_text)
            
            try:
                self.c_cross_score_input.setValue(float(get_text(17)))
            except:
                self.c_cross_score_input.setValue(0)
            
            self.note_input.setText(get_text(18))
        
        except Exception as e:
            QMessageBox.warning(self, "載入資料失敗", f"此列資料格式不完整或有錯誤：\n{str(e)}")
    ...
    def delete_record(self):
        ...
        if row >= 0:
            if QMessageBox.question(self, "確認", "確定刪除？") == QMessageBox.Yes:
                ...
                self.save_data()
                self.update_analysis()
                self.statusBar().showMessage("已刪除選中紀錄", 3000)
        else:
            QMessageBox.warning(self, "提示", "請先點選要刪除的資料列")
        ...
        if item.column() == 1:
            ...
            if cate_item:
                ...
                for r in range(self.config_table.rowCount()):
                    cate_cell = self.config_table.item(r, 0)
                    name_cell = self.config_table.item(r, 1)
                    if cate_cell and name_cell and cate_cell.text() == cate:
                        val = name_cell.text().strip()
                        if val: 
                            new_list.append(val)
    ...
    def apply_conditional_formatting(self):
        ...
        for r in range(self.table.rowCount()):
            try:
                item = self.table.item(r, 15)
                exps.append(float(item.text()) if item else 0)
            except: 
                exps.append(0)
        ...
        for r in range(self.table.rowCount()):
            try:
                if self.table.isRowHidden(r): 
                    continue
                weapon_item = self.table.item(r, 0)
                neck_item = self.table.item(r, 1)
                hourly_item = self.table.item(r, 15)
                crit_item = self.table.item(r, 9)
                if not weapon_item or not neck_item or not hourly_item:
                    continue
                
                combo_key = f"{weapon_item.text()} | {neck_item.text()}"
                hourly = float(hourly_item.text())
                crit = float(crit_item.text().replace('%','')) if crit_item and crit_item.text() else 0
